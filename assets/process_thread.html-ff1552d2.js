import{_ as n,o as s,c as a,f as p}from"./app-9aab0a1e.js";const t={},e=p(`<ul><li><strong>进程</strong>：操作系统可以执行多个任务，进程就是任务，一个进程就相当于一个任务，是操作系统分配资源的最小单位。在 python 中，想要实现多任务可以使用进程来完成，进程是实现多任务的一种方式</li><li><strong>线程</strong>：进程的多个子任务就称之为线程，线程是进程的最小执行单位，一个进程可以有很多线程，每个线程执行的任务都不一样</li></ul><h2 id="多进程-multiprocessing-包" tabindex="-1"><a class="header-anchor" href="#多进程-multiprocessing-包" aria-hidden="true">#</a> 多进程 multiprocessing 包</h2><p>要想更多利用 CPU 资源，可以利用多进程，Python 多进程常用的包 <code>multiprocessing</code>，它拥有很多的功能。比如：子进程、通讯、共享、执行不同的形式等等。</p><h3 id="process——进程类" tabindex="-1"><a class="header-anchor" href="#process——进程类" aria-hidden="true">#</a> Process——进程类</h3><p><code>Process</code> 是 <code>multiprocessing</code> 里面的一个进程类，通过它就能实现多进程。</p><p><strong>语法格式：</strong><code>Process(target[, name[, args[, kwargs]]])</code></p><ul><li><strong>target</strong>：目标，在哪里新开进程让系统去执行，给系统一个目标</li><li><strong>name</strong>：进程的名字，您可以设置也可以不设置，默认是 <code>Process-N</code>，<code>N</code> 是从 <code>1,2,3…N</code>，系统默认从小到大取名</li><li><strong>args</strong> 和 <strong>kwargs</strong>：是参数，可用于传递到目标</li></ul><p>Process 里面有很多方法，其中最常用的就是 <code>start()</code> 方法用于启动进程：<code>进程名.start()</code></p><p>例如：在没有开启多进程时。</p><div class="language-python line-numbers-mode" data-ext="py"><pre class="language-python"><code><span class="token keyword">import</span> time
