...
- **场景一**
    ...
    一个长度为 `10cm` 的面包，每 `3` 分钟吃掉 `1cm`，那么吃掉整个面包需要多久？
...
- **场景二**
    ...
    一个长度为 `16cm` 的面包，每 `5` 分钟吃掉面包剩余长度的一半，即第 `5` 分钟吃掉 `8cm`，第 `10` 分钟吃掉 `4cm`，第 `15` 分钟吃掉 `2cm`…… 那么把面包吃得只剩 `1cm` 需要多久？
...
- **场景三**
    ...
    一个长度为 `10cm` 的面包和一个鸡腿，每 `2` 分钟吃掉 `1` 个鸡腿。那么吃掉整个鸡腿需要多久？
...
- **场景四**：
    ...
    一个长度为 `10cm` 的面包，吃掉第一个 `1cm` 需要 `1` 分钟时间，吃掉第二个 `1cm` 需要 `2` 分钟时间，吃掉第三个 `1cm` 需要 `3` 分钟时间…… 每吃 `1cm` 所花的时间就比吃上一个 `1cm` 多用 `1` 分钟。那么吃掉整个面包需要多久？
...
算法A 运行在老旧电脑上，算法B 运行在某台超级计算机上，超级计算机的运行速度是老旧电脑的 100 倍。
