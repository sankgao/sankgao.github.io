...
例如：将标准输出内容指定到 `nul` 中，`nul` 是一个特殊的设备文件名，‌用于屏蔽命令的输出或错误提示。‌
...
例如：创建一个 `test.cmd` 脚本，使用 `set /p` 命令将原本从屏幕获得的输入信息，改为从指定文件（`file.txt`）获得。内容如下：
