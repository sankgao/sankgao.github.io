...
- **double**：占用 8 个字节（64位），至少提供13位有效数字
...
八进制和十六进制的值，也可以使用后缀 `l` 和 `L` 指定为 `Long` 类型，比如：`020L` 和 `0x20L`。例如：
...
`u` 还可以与其他整数后缀结合，放在前面或后面都可以，比如：10UL、10ULL 和 10LLU 都是合法的。例如：
...
上面示例的运算结果，会输出 positive。原因是变量i和j都是 unsigned int 类型，i - j的结果也是这个类型，最小值为0，不可能得到小于0的结果。正确的写法是写成下面这样。
